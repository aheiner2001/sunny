      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {eq.assetTag && <p className="text-[10px] text-slate-500 mt-0.5">Tag: {eq.assetTag}</p>}
          </div>
        </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Track total owned assets, in-shop inventory, and vehicle assignments.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-sky-600 text-white font-bold text-xs">
            <Plus className="w-4 h-4" />Add Equipment
          </button>
          <Link href="/equipment/scan" className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 text-white font-bold text-xs">
            <QrCode className="w-4 h-4" />Scan Equipment
          </Link>
          <Link href="/issues" className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-500 text-white font-bold text-xs">
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-sm flex flex-col md:flex-row gap-3 items-center justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className="text-xs font-bold text-slate-600">
            View{' '}
            <select value={sortMode} onChange={e => setSortMode(e.target.value as 'equipment' | 'vehicle')} className="px-2 py-1.5 rounded-xl border border-slate-200">
          ...
          <select
            ...
            className="px-2 py-1.5 rounded-xl border border-slate-200 text-xs"
          >
          ...
          {ALL_STATUSES.map((status) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold capitalize ${statusFilter === status ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600'}`}
            >
              {status.replace('_', ' ')}
            </button>
          ))}
          ...
          {groupedByVehicle.map(group => {
            ...
            return (
              <div key={group.id} className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                <button onClick={() => setExpandedVehicles(x => ({ ...x, [group.id]: !expanded }))} className="w-full flex justify-between p-4 text-left">
                  <span>
          ...
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl">
            ...
            <form onSubmit={save} className="space-y-3">
              <input
                ...
                className="w-full px-3 py-2.5 text-xs rounded-xl border border-slate-200"
              />
              <input
                value={form.assetTag}
              ...
              <div className="grid grid-cols-2 gap-2">
                <select
                  ...
                  className="px-3 py-2.5 text-xs rounded-xl border border-slate-200"
                >
                ...
                <input
                  ...
                  className="px-3 py-2.5 text-xs rounded-xl border border-slate-200 disabled:bg-slate-100"
                  placeholder="Total owned"
                />
              </div>
              <select value={form.vehicleId} onChange={e => setForm({ ...form, vehicleId: e.target.value })} className="w-full px-3 py-2.5 text-xs rounded-xl border border-slate-200">
                <option value="">In shop / unassigned</option>
                {vehicles.map(v => <option key={v.id} value={v.id}>{v.vehicleNumber} - {v.name}</option>)}
              </select>
              <div className="grid grid-cols-2 gap-2">
                <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value as EquipmentCategory })} className="px-3 py-2.5 text-xs rounded-xl border border-slate-200">
                  <option value="equipment">Equipment</option>
                  <option value="supplies">Supplies</option>
                  <option value="vehicle_condition">Vehicle condition</option>
                </select>
                <select value={form.status} onChange={e => setForm({ ...form, status: e.target.value as EquipmentStatus })} className="px-3 py-2.5 text-xs rounded-xl border border-slate-200">
              ...
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setModal(null)} className="flex-1 py-2.5 rounded-xl border text-xs font-bold">Cancel</button>
                <button disabled={loading} className="flex-1 py-2.5 rounded-xl bg-sky-600 text-white text-xs font-bold">{loading ? 'Saving...' : 'Save'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {modal === 'delete' && selected && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full text-center shadow-2xl">
            ...
            <p className="text-xs text-slate-500 my-3">This removes the inventory record and its assignments.</p>
            <div className="flex gap-2">
              <button onClick={() => setModal(null)} className="flex-1 py-2.5 rounded-xl border text-xs font-bold">Cancel</button>
              <button onClick={remove} disabled={loading} className="flex-1 py-2.5 rounded-xl bg-rose-600 text-white text-xs font-bold">Delete</button>
            </div>
          </div>
        </div>
      )}
