...
import { AVATAR_PRESETS, findAvatarPresetByUrl, getResolvedAvatarUrl } from '@/lib/avatarPresets';
...
export function ProfileModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const [avatarChoice, setAvatarChoice] = useState<'none' | 'custom' | string>('none');
  ...
  useEffect(() => {
    if (user && isOpen) {
      ...
      const preset = findAvatarPresetByUrl(user.avatarUrl);
      setAvatarChoice(user.avatarUrl ? (preset?.id || 'custom') : 'none');
